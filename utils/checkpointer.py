...
class Checkpointer:
    ...
    def __init__(self, cfg, logger, model, **checkpointables):
        ...
        self.logs = (
            self._load_log()
        )  # if there is no existing logs, get dict with None values
    ...
    def load_resume(self, pretrained_w=None):
        ...
        """
        assert pretrained_w is None, "currently do not support pretrained_weights"
        logs = self.logs
        last_cp_iter = 0
        ...
        assert (
            last_cp_iter <= last_data_iter and last_cp_iter >= last_data_iter - 1
