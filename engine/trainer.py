...
from modelling import ModelTrainer, get_dataset, get_model
from utils import Checkpointer, Timer, setup_listen_logger
...
class Trainer:
    ...
    def __init__(self, cfg, resume=False):
        ...
        self.checkpointer = Checkpointer(cfg, logging.getLogger(), self.model)
    ...
    def compare_models(self):
        ...
        if (
            new_wins + old_wins == 0
            or float(new_wins) / (new_wins + old_wins) < self.cfg.SOLVER.UPDATE_THRESH
        ):
